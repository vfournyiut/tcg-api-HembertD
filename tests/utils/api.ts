...
export function createExpiredToken(userId: number = 1, email: string = 'test@example.com'): string {
...
const CARD_DATA: Record<number, Omit<MockCard, 'id' | 'pokedexNumber' | 'imgUrl' | 'createdAt' | 'updatedAt'>> = {
...
export function createMockCard(id: number, overrides: Partial<MockCard> = {}): MockCard {
    const data = CARD_DATA[id] || {name: `Card${id}`, type: 'Normal', hp: 50, attack: 50};
    return {
        id,
        name: data.name,
        pokedexNumber: id,
...
export function createMockDeckCard(deckId: number, cardId: number, index: number): object {
    return {
        id: index,
        deckId,
        cardId,
...
export function createMockDeck(userId: number = 1, overrides: Partial<MockDeck> = {}): MockDeck {
    return {
        id: 1,
        name: 'Mon Deck',
        userId,
...
export function createMockDeckWithCards(userId: number = 1): MockDeck {
    ...
    return {
        ...
        createdAt: new Date(),
        updatedAt: new Date(),
        cards: cards.map((card, index) => createMockDeckCard(1, card.id, index + 1))
    } as MockDeck;
}
...
export function mockError(prismaMock: DeepMockProxy<PrismaClient>, method: keyof DeepMockProxy<PrismaClient>, errorMessage: string = 'Database error'): void {
    (prismaMock[method] as any).mockRejectedValue(new Error(errorMessage));
}

/** Configure le mock pour retourner un tableau vide */
export function mockEmpty(prismaMock: DeepMockProxy<PrismaClient>, method: keyof DeepMockProxy<PrismaClient>): void {
